...
3. main 브랜치에서 새로운 브랜치를 생성해 작업합니다. (예: `feat/bookmark`)

   - **문서(docs) 기여의 경우**  
     작업 중인 브랜치가 다른 기여자의 브랜치와 충돌하지 않도록  
     `docs/작성자명/주제` 형식으로 브랜치 이름을 명확하게 설정하는 것을 권장합니다. (예: `docs/waterbinnn/js-array`)  

      ※ 여러 개의 문서를 하나의 브랜치에서 작성하는 경우, 주제 부분에는 보다 포괄적인 범주 또는 작성 목적을 사용하는 것을 추천합니다. (예: `docs/waterbinnn/js`)
...
- 문서(docs) 기여 시에는 가능하면 문서 1건당 이슈를 하나씩 작성해주세요.
