...
HTTP는 1.0부터 시작해 1.1, 2.0, 3.0에 이르기까지 계속해서 발전해왔습니다. 
오늘은 HTTP 버전별 특징과 이전 버전의 단점을 어떻게 극복해 왔는지 살펴보겠습니다.
...
- 파이프라이닝 개념 도입
  - 1개의 TCP 연결에 1:1 요청/응답만 가능했던 이전 버전에서 개선
  - 단일 TCP 연결에서서 여러 개의 요청을 보내는 것이 가능
